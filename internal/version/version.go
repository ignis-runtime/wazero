...
//
// If this is not CLI, this assumes that downstream users of wazero imports wazero as "github.com/ASparkOfFire/wazero".
// To be precise, the returned string matches the require statement there.
// For example, if the go.mod has "require github.com/ASparkOfFire/wazero 0.1.2-12314124-abcd",
...
func GetWazeroVersion() (ret string) {
	...
	if ok {
		for _, dep := range info.Deps {
			// Note: here's the assumption that wazero is imported as github.com/ASparkOfFire/wazero.
			if strings.Contains(dep.Path, "github.com/ASparkOfFire/wazero") {
				ret = dep.Version
			}
		}
